...
abstract public class WakefulIntentService extends IntentService {
    ...
    public static void sendWakefulWork(Context ctxt, Class<?> clsService) {
        sendWakefulWork(ctxt, clsService, null);
    }

    public static void sendWakefulWork(Context ctxt, Class<?> clsService, Bundle bundle) {
        Intent _intent = new Intent(ctxt, clsService);
        loadIntent(_intent, bundle);
        sendWakefulWork(ctxt, _intent);
    }

    private static void loadIntent(Intent intent, Bundle bundle) {
        if (bundle == null) return;
        if (bundle.containsKey("action")) intent.setAction(bundle.getString("action"));
        if (bundle.containsKey("booleansKeys")) {
            ArrayList<String> booleans = bundle.getStringArrayList("booleansKeys");
            boolean[] booleanValues = bundle.getBooleanArray("booleansValues");
            if (booleans.size() != booleanValues.length) throw new IllegalArgumentException("Array length must match list size");
            int i = 0;
            for (String key : booleans) intent.putExtra(key, booleanValues[i ++]);
        }
    }
